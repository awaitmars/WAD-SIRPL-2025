...
<head>
    ...
    <title>SI-RP | Validasi Jadwal Dosen</title>
...
<body class="bg-slate-50 text-slate-800">
    ...
    <div class="flex min-h-screen">
        <aside class="w-72 bg-slate-900 text-white hidden md:block fixed h-full z-20">
            ...
            <nav class="mt-6 px-4">
                <a href="#" class="flex items-center px-4 py-3 bg-indigo-600 text-white rounded-lg shadow-lg">
                    <i class="fas fa-clock mr-3 text-yellow-300"></i> Jadwal & Validasi
                </a>
            </nav>
        </aside>
        ...
        <main class="flex-1 md:ml-72 p-8 overflow-y-auto">
            <header class="flex justify-between items-center mb-8 bg-white p-4 rounded-xl shadow-sm border border-slate-200">
                <div>
                    ...
                    <p class="text-sm text-slate-500">Integrasi jadwal akademik dengan API Jadwal Sholat.</p>
                </div>
                <div class="flex items-center space-x-3">
                    <div class="text-right">
                        <p class="text-sm font-bold">Dr. Budi Santoso, M.T.</p>
                        <p class="text-xs text-indigo-600">NIDN: 0412345678</p>
                    </div>
                    <div class="w-10 h-10 rounded-full bg-indigo-100 flex items-center justify-center text-indigo-700 font-bold">BS</div>
                </div>
            </header>

            @if(session('success'))
            <div class="mb-6 bg-green-50 border-l-4 border-green-500 p-4 text-green-700 rounded-r shadow-sm">
            ...
            <div class="mb-6 bg-red-50 border-l-4 border-red-500 p-4 text-red-700 rounded-r shadow-sm">
                <p class="font-bold">Terjadi Kesalahan Input / API Error</p>
            ...
            <div class="{{ $isApiOnline ? 'bg-indigo-50 border-indigo-100' : 'bg-rose-50 border-rose-100' }} border rounded-2xl p-5 mb-8 flex items-center justify-between transition-colors duration-300">
                <div class="flex items-start space-x-4">
                    <div class="p-3 bg-white rounded-xl shadow-sm">
                        <i class="fas fa-satellite-dish {{ $isApiOnline ? 'text-indigo-600' : 'text-rose-500' }}"></i>
                    </div>
                    <div>
                        ...
                        <p class="{{ $isApiOnline ? 'text-indigo-900' : 'text-rose-900' }} font-medium">
                            Target: <span class="font-bold">api.myquran.com</span>
                        </p>
                    </div>
                </div>
                ...
                @else
                    <span class="px-3 py-1 bg-red-100 text-red-700 text-xs font-bold rounded-full uppercase flex items-center">
                        <i class="fas fa-exclamation-triangle mr-2"></i> Offline / Error
                    </span>
                @endif
            </div>
            ...
            <div class="flex justify-between items-center mb-6">
                ...
                <div class="flex space-x-3">
                    <a href="{{ route('jadwal.exportPdf') }}" target="_blank" class="bg-rose-600 hover:bg-rose-700 text-white px-5 py-3 rounded-xl font-bold shadow-lg transition flex items-center">
                        <i class="fas fa-file-pdf mr-2"></i> Download PDF
                    </a>
                    <button onclick="openNewPlanModal()" class="bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-3 rounded-xl font-bold shadow-lg transition flex items-center">
                        ...
                        <tbody class="divide-y divide-slate-100">
                            ...
                            <tr class="hover:bg-slate-50 transition">
                                ...
                                <td class="px-6 py-5 text-center">
                                    ...
                                    @else
                                        <div class="inline-flex items-center space-x-1.5 px-3 py-1 bg-amber-50 text-amber-600 rounded-full text-[10px] font-bold uppercase">
